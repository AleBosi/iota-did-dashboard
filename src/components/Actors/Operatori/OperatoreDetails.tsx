...
const OperatoreDetails: React.FC<Props> = ({ operatore }) => {
  ...
  const did = operatore.did || "-";
  const addr = operatore.evmAddress;
    ...
    <div className="bg-white rounded-2xl shadow p-6 mb-4">
      ...
      <div className="grid gap-2 text-base">
        ...
        {operatore.seed && (
          <div>
            <b>Seed:</b>{" "}
            <code className="bg-gray-100 rounded px-2">{operatore.seed}</code>
          </div>
        )}
        ...
        {operatore.publicKey && (
          <div>
            <b>PublicKey:</b>{" "}
            <code className="bg-gray-100 rounded px-2">{operatore.publicKey}</code>
          </div>
        )}
        ...
        {operatore.aziendaId && (
          <div>
            <b>Azienda (ID):</b>{" "}
            <span className="text-xs text-gray-500">{operatore.aziendaId}</span>
          </div>
        )}
