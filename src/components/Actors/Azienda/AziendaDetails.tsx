...
export default function AziendaDetails({ azienda }: Props) {
  ...
  return (
    <div className="border rounded p-4 bg-gray-50 mb-2">
      ...
      <div><b>Creatori:</b> {azienda.creators.length}</div>
      <div><b>Operatori:</b> {azienda.operatori.length}</div>
      <div><b>Macchinari:</b> {azienda.macchinari.length}</div>
      <div className="text-xs text-gray-400">Creato il: {azienda.createdAt}</div>
    </div>
  );
}
